...
class SocketService {
        ...
        this.io.on('connection', (socket) => {
            ...
            socket.on('subscribe:device:status', async (deviceId) => {
                try {
                    ...
                    socket.emit('error', { message: 'Failed to subscribe to device status', error: error.message });
                }
            });
            ...
            socket.on('subscribe:device:progress', async (deviceId) => {
                try {
                    ...
                    socket.emit('error', { message: 'Failed to subscribe to device progress', error: error.message });
                }
            });
            ...
            socket.on('subscribe:device:errors', async (deviceId) => {
                try {
                    ...
                    socket.emit('error', { message: 'Failed to subscribe to device errors', error: error.message });
                }
            });
    ...
    async subscribeToDeviceErrors(socket, deviceId) {
        ...
        console.log(`🚨 Client ${socket.id} subscribed to error notifications for ${room}`);
    ...
    async sendRecentErrors(socket, deviceId) {
        try {
            ...
            console.error(`❌ Error sending recent errors for device ${deviceId}:`, error);
    ...
    async notifyDeviceError(deviceId, errorData) {
        try {
            ...
            console.error(`❌ Error broadcasting device error for ${deviceId}:`, error);
        }
    }
}
